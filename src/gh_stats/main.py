...
def main():
    ...
    parser.add_argument('--personal-limit', type=int, default=20, help='Max personal repos to scan (default: 20)')
    parser.add_argument('--org-limit', type=int, default=50, help='Max repos per org to scan (default: 50)')
    ...
    limits_info = []
    if args.personal and args.personal_limit > 0: limits_info.append(f"personal: {args.personal_limit}")
    if orgs and args.org_limit > 0: limits_info.append(f"org: {args.org_limit}")
    ...
    if args.personal:
        ...
        user_repos = get_user_repos(username, args.personal_limit if args.personal_limit > 0 else None)
    ...
    for org in orgs:
        ...
        org_repos = get_org_repos(org, args.org_limit if args.org_limit > 0 else None)
